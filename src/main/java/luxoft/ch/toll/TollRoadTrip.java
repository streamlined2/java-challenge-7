...
public class TollRoadTrip {
	...
	public List<Choice> solve(Integer money, Integer coupons) {
		...
		int level = choose(solution, 0, money, coupons);
		return Arrays.asList(Arrays.copyOf(solution, level + 1));
	}

	private int choose(Choice[] choices, int level, Integer money, Integer coupons) {
		if (level >= costs.size()) {
			return costs.size() - 1;
		}
		...
		if (money >= currentCost) {
			moneyLevel = choose(choices, level + 1, money - currentCost, coupons);
		}
		int couponLevel = -1;
		if (coupons >= 1) {
			couponLevel = choose(choices, level + 1, money, coupons - 1);
		}
	...
	public static void main(String[] args) {
		TollRoadTrip trip = new TollRoadTrip(List.of(100, 50, 25, 12, 6, 3, 2, 1, 20, 30));
